...
async function inicializarBaseDeDatos() {
    ...
    if (!localStorage.getItem('productosLocal') || JSON.parse(localStorage.getItem('productosLocal')).length === 0) {
        try {
    ...
    // 2. CREAR USUARIOS SEMILLA SÓLO SI LA TABLA NO EXISTE EN ABSOLUTO
    if (!localStorage.getItem('usuarios')) {
        const adminPredeterminado = [
            { name: "Admin UCAB", email: "admin@ucab.ve", password: "123", role: "Administrador", avatar: "https://picsum.photos/100", address: "Sede Montalbán" },
            { name: "Cliente Demo", email: "cliente@ucab.ve", password: "123", role: "Cliente", avatar: "https://picsum.photos/100", address: "Caracas, Venezuela" }
        ];
        localStorage.setItem('usuarios', JSON.stringify(adminPredeterminado));
        console.log('👥 Usuarios semilla inicializados con éxito.');
    }

    // 3. Inicializar tablas de soporte vacías si no existen
